...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add') {
        ...
        if ($data['course_id'] && $data['semester'] && $data['academic_year'] && $data['amount'] >= 0) {
            if ($feeObj->structureExists($data['course_id'], $data['semester'], $data['academic_year'])) {
                $error = 'A fee structure already exists for that course, semester, and year.';
            } elseif ($feeObj->createStructure($data)) {
                header("Location: structures.php?msg=added");
                exit();
            } else {
                $error = 'Failed to create fee structure. Please try again.';
            }
        } else {
    ...
    if ($_POST['action'] === 'delete') {
        ...
        if ($id && $feeObj->deleteStructure($id)) {
            header("Location: structures.php?msg=deleted");
            exit();
        }

        $error = 'Unable to delete the selected fee structure.';
    }
}
                ...
                <tbody>
                    <?php foreach ($structures as $st): ?>
                        <tr>
                            ...
                            <td>TZS <?php echo number_format($st['amount'], 2); ?></td>
                            <td>
